...
Your [wid_id] and temp password is in the email that IT sent you (it is NOT your NetID). For example:

`>> ssh elee1@roy-submit.discovery.wisc.edu`

```{r servers, echo=FALSE}
servers <-read.csv('roy-servers.csv',header=TRUE)
colnames(servers) =c("Server","RAM (GB)","# of CPUs", "Scratch storage (TB)", "% dedicated to Condor")
knitr::kable(servers, caption = 'List of servers')
```
...
### Python {#python}
* For external programs: other researchers may have released their programs in Python. We install them in a **conda** environment (using **pip**) or by downloading them from git.
...
As you work on a specific project, Sushmita or someone will let you know under which elog list you can document your experiments and results in.
...
### pages.discovery.wisc.edu/~[wid_id] {#pages}

Everyone with a WID ID gets a free webpage and space at pages.discovery.wisc.edu/~[wid_id]. You can actually copy in image files into this space from any of our servers, and look at it on a browser ([example](http://pages.discovery.wisc.edu/~elee1/eigen_elbow_test/)):
